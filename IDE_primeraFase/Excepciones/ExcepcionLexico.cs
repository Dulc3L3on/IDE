...
namespace IDE_primeraFase.Excepciones
{
    class ExcepcionLexico{//en esta clase se encontrarán todos los métodos que corresponderán a cada uno de los análisis que reailza el léxico, aquí se llamará al método que interactúa directametne con la interfaz para que así pueda colorearse la fila en la se estaba analizando [donde se encontraba el error] y para que así se actualice la lista que se muetra en el log...                      
        ...
        public String excepcionPalabra(int numeroFila, int columnaExcepcion, char[] lineaCompleta){//puesto que aún no han sido solicitados los identificadores, entones no agrego la parte donde si es un # ó _ entonces lo paso a identificador de lo contrario a lista de errores...
