...
using IDE_primeraFase.Excepciones;
using IDE_primeraFase.Herramientas;

namespace IDE_primeraFase
{//en esta parte se encontrarán todos los métodos para realizar los análisis por cada
//tipo de ocurrencia que surga, donde al no existir en el alfabeto se tomará como una
//entrada errónea

    class AnalizadorLexico//recuerda que luego de tener al actual que se evaluará, debe revisarse el siguiente para saber si se debe seguir en el método o se debe entrar en otra ramificación [en el caso de los símbolos] para detectar si dicho conjunto cumple o no...
    {
        Herramienta herramienta = new Herramienta();
        ExcepcionLexico excepcionLexico = new ExcepcionLexico();
        TablaSimbolos tablaSimbolos = new TablaSimbolos();

        public void analizar(char[] lineaDesglosada, int numeroLinea) {//Aquí ya actuó el método sea que aquí mismo se encuantre o lo esté en la clase que tiene contacto directo con la interfaz...
            Resultado resultadosHallados = new Resultado();

            for (int caracterActual=0; caracterActual< lineaDesglosada.Length; caracterActual++) {
                if (!((int)lineaDesglosada[caracterActual] == 32)) { //Es decir es un espacio en blanco...
        ...
        private String[] analizarPalabra(int numeroLinea, int posicionInicialAnalisis, char[] lineaAEstudiar) {//recuerda que por el hecho de recibir la línea almacenada en un arr, entonces si el número que corresponda a las col será uno menos a lo ordinario, pero debes averiguar como trabaja con las col el richTextBox... para que cuando necesites hacer manipulaciones alguna vez, no surgan problemas...
            ...
            while ((int)lineaAEstudiar[posicionAnalisis] != 32 && (parar==false) && (posicionAnalisis< lineaAEstudiar.Length)) {//Es decir mientras no halle un espacio en blanco...      para cuando ya se agregue el identificador esta condi de !palabra deberá cambiar puesto que sin importar que sea palabra o identificador, deberá parar... ahi si deberá usarse un bool, que cb de val para que ya no siga, media vez entre al if para usar el método de Excepción...
        ...
        public void analizarSimbologia() { 
            //lo que decidiste fue, si había un sigueinte == 32 entnces entrarías al método más peq, el de fin y asignación, sino entonces de una vez entrarías a la sigueinte ramificación
            //Donde revisarías si es / entonces te pondrías a revisar si es div, coment linea o coment multi... sino pues en la enum
        }

        public void analizarDatosPrimitivos() {
        
        }

        public void analizarOperadores() { 
        ...
        public void patronCadena() {

            compararConPalabrasReservadas();
        }

        private void compararConPalabrasReservadas()
        {


        }

        public void patronCaracter() { 
        
        }

        public void patronBooleano() { 
...
//el analizador léxico también corrobora que no hayan identificadores repetidos???